...
// ---- TEMP DEBUG: MediaRecorder-MP4 verification helpers (remove before commit) ----
// SW console usage: mp4force() → record + edit → mp4test().  mp4reset() restores.
try {
  globalThis.mp4force = async () => {
    await chrome.storage.local.set({ useWebCodecsRecorder: false });
    console.log("[mp4] fast recorder OFF, MediaRecorder path forced. Record now.");
  };
  globalThis.mp4reset = async () => {
    await chrome.storage.local.set({ useWebCodecsRecorder: true });
    console.log("[mp4] fast recorder ON, back to WebCodecs default.");
  };
  globalThis.mp4test = async () => {
    const s = await chrome.storage.local.get([
      "diagnosticLog", "editorReadyPath", "lastRecordingError",
      "useWebCodecsRecorder", "lastRecordingBackendRef", "__mp4dbg", "__hiResResult",
    ]);
    const ses = s.diagnosticLog?.sessions?.slice(-1)[0];
    const evs = ses?.events || [];
    const lastEv = (e) => [...evs].reverse().find((x) => x.e === e);
    const has = (e) => evs.some((x) => x.e === e);
    const ok = (b) => (b ? "✅" : "❌");

    // backend is the ground truth (single storage key, never rotates):
    // opfs = WebCodecs, idb = MediaRecorder. recorder-finalize-summary is a
    // WebCodecs-only session marker that corroborates it.
    const backend = s.lastRecordingBackendRef?.backend || null;
    const ranMR = backend === "idb" && !has("recorder-finalize-summary");
    const recorder = ranMR ? "MediaRecorder" : backend === "opfs" || has("recorder-finalize-summary") ? "WebCodecs" : "unknown";

    // Durable mime written by createMediaRecorder (survives lifecycle-log rotation).
    const mime = s.__mp4dbg?.mime || null;
    const container = s.__mp4dbg?.container || null;
    const mp4Container = container === "mp4";
    const path = lastEv("editor-load-ready")?.d?.path || s.editorReadyPath || null;
    const mp4Route = path === "mp4-fast";
    const meta = lastEv("sandbox-video-loadedmetadata")?.d || {};
    const durOk = Number.isFinite(meta.duration) && meta.duration > 0;
    const errs = evs.filter((x) => /error/i.test(x.e)).map((x) => x.e);
    const pass = ranMR && mp4Container && mp4Route && durOk && !errs.length;

    console.log("%c── Screenity MR-MP4 test ── " + (pass ? "PASS ✅" : ranMR ? "CHECK ❌" : "WRONG RECORDER ⚠️"), "font-weight:bold");
    console.log("recorder that ran:", recorder, "(backend:", backend + ")", ok(ranMR),
      ranMR ? "" : "→ not the MediaRecorder path; turn OFF \"Fast MP4\" then record a NEW clip");
    console.log("(a) container:", container, ok(mp4Container), " [mime:", mime, "]", mp4Container ? "→ stayed MP4" : "→ fell back to WebM");
    console.log("(b) editor route:", path, ok(mp4Route), mp4Route ? "→ MP4 fast path" : "→ WebM/remux path");
    console.log("    duration:", meta.duration, ok(durOk), " dims:", meta.width + "x" + meta.height);
    console.log("    editor errors:", errs.length ? errs : "none", ok(!errs.length));
    console.log("    lastRecordingError:", s.lastRecordingError || "none");
    console.log("    Fast-MP4-off flag:", s.useWebCodecsRecorder, " session:", ses?.id, ses?.config?.recordingType, "→", ses?.outcome);
    const hr = s.__hiResResult;
    if (hr) {
      console.log("%c    capture track:", "color:#888",
        hr.width + "x" + hr.height, hr.displaySurface || "",
        "| requested", hr.requested, "| hiResTest:", hr.hiResTest);
    }
    return pass;
  };
} catch {}
